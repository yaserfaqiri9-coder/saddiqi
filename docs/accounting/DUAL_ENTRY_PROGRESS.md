...
> آخرین به‌روزرسانی: 2026-07-17 (مراحل ۱۲ تا ۱۵ کامل شد)
...
| ۱۵ | بازگشایی کنترل‌شده | ✅ کامل (Migration ساخته‌شده، اجرانشده) |
...
برمی‌گردند و هیچ سندی Delete/ویرایش نمی‌شود.

### تغییرات مدل + Migration (اجرانشده)
- `FiscalYear` — `ReopenedAt`, `ReopenedByUserId`, `ReopenReason` (تاریخچهٔ `ClosedAt/By` حفظ می‌شود).
- `RoleAccessRules.AppPermissions.ReopenFiscalYear` — Permissionِ صریح (از نقش استنتاج نمی‌شود؛ حتی Admin بدونش نمی‌تواند).
- Migration: `20260716211814_AddFiscalYearReopenFields` (فقط جدول `FiscalYears`). **اجرا نشده.**

### فایل‌های جدید
- `Services/Accounting/ReopenModels.cs`، `Services/Accounting/ReopenFiscalYearService.cs`
- `Controllers/ReopenFiscalYearController.cs`، `Views/ReopenFiscalYear/Index.cshtml`
- `Models/Accounting/ReopenPageViewModel.cs`
- `tests/PTGOilSystem.Web.Tests/ReopenFiscalYearServiceTests.cs` (۱۱ تست، همه سبز)

### قواعد پیاده‌شده
- فقط **آخرین** سالِ بسته قابل بازگشایی است (وجودِ سالِ بسته‌ی بعدی → `LATER_CLOSED_YEAR_EXISTS`).
- سالِ بعد اگر Postingِ عملیاتی (SourceModule ≠ FiscalYearClose) داشته باشد → مانع؛ سندهای سیستمیِ
  Opening/Revaluation به‌تنهایی مانع نیستند.
- AdminOnly + Permissionِ `ReopenFiscalYear` + Reason اجباری + عبارتِ تأییدِ شاملِ کد سال + Audit.
- POST + antiforgery؛ هیچ عملیاتِ تغییردهنده با GET نیست.

### ترتیب اتمیک (داخل یک Transaction)
1. Preconditions.
2. سال → `Reopened`, `IsCurrent=true`, ثبتِ `ReopenedAt/By/Reason` (تاریخچهٔ Closed حفظ).
3. **فقط آخرین دوره** Open می‌شود؛ دوره‌های قبلی HardLocked می‌مانند — تا Reversalِ سندهای بستن
   (به تاریخ EndDate) از PeriodGuard عبور کند.
4. Reverseِ رسمیِ همهٔ سندهای CloseRun (P&L و Retained Earnings) — بدون حذف.
5. `FiscalYearCloseRun` با `FailureCode="REOPENED"` علامت می‌خورد (تاریخچه پاک نمی‌شود).
6. سالِ بعد در صورتِ نبودِ Postingِ عملیاتی → `Draft` و `IsCurrent=false`.
7. Audit. هر شکستی → Rollback (و Reason Code `REOPEN_REVERSAL_FAILED`).

### دامنهٔ Reverse
فقط سندهای Final Close برمی‌گردند. جفتِ Revaluation + Auto-Reversal متعلق به Trial Close است و
خودبه‌خود نت می‌شود؛ دست‌نخورده می‌ماند تا اجرای دوبارهٔ Trial/Final با منطقِ Revisionِ مرحله ۱۳
آن را دوباره ارزیابی کند. بعد از بازگشایی، Closeِ جدید Revisionِ جدید می‌سازد و CloseRunِ قدیمی را
بازنویسی نمی‌کند.

### Idempotency
سالِ از قبل بازگشایی‌شده → `AlreadyReopened` بدون Reversalِ جدید (تست‌شده).
...
## تأیید نهاییِ مراحل ۱۲ تا ۱۵ (۲۰۲۶-۰۷-۱۷)

### SHA چهار Commit (لوکال — Push توسط کاربر انجام می‌شود؛ اعتبارنامهٔ فعلی اجازهٔ Push نداشت: 403)
- مرحله ۱۲ — `c027333` `feat(accounting): add fiscal year closing checklist`
- مرحله ۱۳ — `b3f7250` `feat(accounting): add trial close and period-end revaluation`
- مرحله ۱۴ — `535538a` `feat(accounting): implement controlled fiscal year final close`
- مرحله ۱۵ — `f1dde2a` `feat(accounting): add controlled fiscal year reopening`

### Build
- `dotnet build -c Release`: **۰ خطا** (روی Working Tree که شاملِ فایل‌های UIِ Commit‌نشدهٔ کاربر است).
- ⚠️ Worktreeِ تمیز از HEAD **به‌تنهایی Build نمی‌شود**، ولی این ربطی به این کار ندارد:
  `ContractsController.cs` (Commit قبلیِ `4480665`، پیش از این کار) به اعضای
  `ActiveCount/PurchaseCount/SaleCount` نیاز دارد که فقط در نسخهٔ **Commit‌نشدهٔ** کاربر از
  `ContractIndexViewModel.cs` وجود دارند. یعنی repo هم‌اکنون به تغییراتِ Commit‌نشدهٔ کاربر وابسته
  است. طبق قانون، آن فایل‌ها لمس/Commit نشدند.

### تست هدفمند هر مرحله (In-Memory، همه سبز)
- مرحله ۱۲ `ClosingChecklistServiceTests`: **۱۷/۱۷**
- مرحله ۱۳ `TrialCloseServiceTests`: **۱۴/۱۴**
- مرحله ۱۴ `FinalCloseServiceTests`: **۱۲/۱۲**
- مرحله ۱۵ `ReopenFiscalYearServiceTests`: **۱۱/۱۱**
- جمع: **۵۴ تست جدید، همه سبز.**

### Full Suite

#### تأییدِ نهایی روی Worktree کاملاً تمیز (Commit `3c906a8`)
پیش از این، نسخهٔ تمیزِ HEAD به‌تنهایی Build نمی‌شد: `ContractsController.cs` (از Commit `4480665`)
به سه عضوِ `ActiveCount`/`PurchaseCount`/`SaleCount` از `ContractIndexViewModel` وابسته بود که فقط
در Working Tree کاربر وجود داشتند (خطای `CS0117`). در Commit `3c906a8` **فقط همان سه Property**
Commit شد؛ سایر تغییرات UI کاربر دست‌نخورده در Working Tree باقی ماند.

روی Worktree تمیزِ ساخته‌شده از `3c906a8` (بدون هیچ فایلِ Commit‌نشدهٔ کاربر):
- `dotnet restore` + `dotnet build ptg-oil-system.sln -c Release` → **۰ خطا / ۴ هشدارِ قدیمی**.
  → **Clean HEAD اکنون بدون کمکِ Working Tree کاربر Build می‌شود.**
- تست‌های هدفمندِ مراحل ۱۲ تا ۱۵: **۵۴ / ۵۴ سبز** (۰ شکست).
- `git diff --check` روی بازهٔ `c027333^..HEAD`: **بدون خطای Whitespace**.

#### شکست‌ها
- اجرا با فیلترِ غیر-PG: **۱۰۳۶ Pass / ۳۸۴ Fail / ۱۴۲۰ Total**.
- **۳۶۸ شکست (۱۸۴ تستِ یکتا)** متعلق به ۱۲ کلاسِ Collectionِ «Accounting PostgreSQL» است که
  نامشان شاملِ `PostgreSql` نیست و به همین دلیل با فیلترِ سادهٔ نام حذف نمی‌شوند. این‌ها فقط به‌خاطرِ
  **نبودِ سرورِ PostgreSQL** شکست می‌خورند (`PTG_TEST_POSTGRES_ADMIN` تنظیم نشده) — محدودیتِ محیط،
  نه Regression. وضعیت: **Not Executed**.
- **شکست‌های واقعیِ غیر-PG: دقیقاً ۱۶ تست** — همان Baselineِ ۱۶‌تایی، بدون کم‌وکاست.

#### اثباتِ صفر Regression
همان اجرا روی Worktreeِ Baseline (`c027333^` = `8212414`، پیش از مرحلهٔ ۱۲) تکرار شد و مجموعهٔ
شکست‌ها **نام‌به‌نام** مقایسه شد:

| | Baseline (`8212414`) | HEAD (`3c906a8`) |
|---|---|---|
| شکستِ واقعیِ غیر-PG | ۱۶ | ۱۶ |
| شکستِ Collectionِ PostgreSQL | ۱۸۴ | ۱۸۴ |

`comm` روی دو فهرستِ مرتب‌شده: **صفر شکستِ جدید، صفر شکستِ برطرف‌شده** → مجموعه‌ها یکسان‌اند.
→ **مراحل ۱۲ تا ۱۵ هیچ Regression ایجاد نکرده‌اند.**

> نکتهٔ اصلاحی: در گزارشِ قبلی عددِ «۱۵ شکستِ غیر-PG» ثبت شده بود. آن اجرا در Working Tree کاربر
> انجام شده بود و یکی از تست‌های View-Structure به‌کمکِ فایل‌های UIِ Commit‌نشدهٔ کاربر سبز می‌شد.
> روی Worktree تمیز عدد درست **۱۶** است. **Baseline واقعی = ۱۶.**

#### اجرای Collectionِ PostgreSQL (اجرا نشد)
- متغیرِ محیطیِ لازم: **`PTG_TEST_POSTGRES_ADMIN`** — یک Connection Stringِ ادمین، مثلاً
  `Host=...;Port=5432;Username=...;Password=...;Database=postgres`.
  (پیش‌فرضِ کد `Host=localhost;Port=5432;Username=postgres;Password=postgres;Database=postgres` است
  که در این محیط معتبر نیست.)
- فرمانِ اجرا:
  ```
  dotnet test tests/PTGOilSystem.Web.Tests/PTGOilSystem.Web.Tests.csproj -c Release \
    --filter "FullyQualifiedName~AccountingCorePostgreSqlTests|FullyQualifiedName~AccountingReversalTests|FullyQualifiedName~ContractBalanceTransferAccountingAdapterTests|FullyQualifiedName~ExpenseAccountingAdapterTests|FullyQualifiedName~InventoryTransferAccountingAdapterTests|FullyQualifiedName~PaymentAccountingAdapterTests|FullyQualifiedName~PaymentCompanyOwnershipTests|FullyQualifiedName~PaymentCompanyResolverTests|FullyQualifiedName~PurchaseAccountingAdapterTests|FullyQualifiedName~SalesAccountingAdapterTests|FullyQualifiedName~Stage8AccountingAdapterTests|FullyQualifiedName~SupplierPaymentAllocationAccountingAdapterTests|FullyQualifiedName~ViaSarrafAccountingAdapterTests"
  ```
- **وضعیت: Not Executed** — اعتبارنامهٔ PostgreSQL در این محیط موجود نبود. هیچ اعتبارنامه‌ای اختراع
  یا Commit نشد.

### Migrationهای ساخته‌شده — **هیچ‌کدام اجرا نشده**
- `20260716203917_AddMonetaryTreatmentAndCloseRunSnapshot` (جدول‌های `Accounts` و `FiscalYearCloseRuns`).
- `20260716211814_AddFiscalYearReopenFields` (جدول `FiscalYears`).
- اجرا فقط با تصمیم صریح و روی دیتابیس هدف:
  `dotnet ef database update --project src/PTGOilSystem.Web`.

### Feature Flagها
همهٔ Flagهای Accounting **خاموش** باقی ماندند؛ هیچ Flag روشن نشد.

### فایل‌های UIِ Commit‌نشدهٔ کاربر
هیچ‌کدام لمس، Reset، Checkout، Clean، Stash یا Commit نشدند.

### آخرین نقطهٔ توقفِ پروژه
**پایانِ مرحله ۱۵.** مراحل ۰ تا ۱۵ کامل‌اند. باقی‌مانده: (۱) Push چهار Commit توسط کاربر،
(۲) اجرای Migrationها روی دیتابیس هدف با تصمیم صریح، (۳) اجرای Full Suite در محیطِ دارای
اعتبارنامهٔ PostgreSQL برای تأییدِ نهاییِ Collectionِ یکپارچه، (۴) روشن‌کردنِ Flagها در Cutover.
